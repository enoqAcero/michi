	...
	if walking == true:
		...
		if movingVerticalHorizontal == 1:
			...
			if xdir == -1:
				...
				$AnimatedSprite2D.position = Vector2(37,-38)
			if xdir == 1:
				...
				$StatusGood.position.y = -36
				$AnimatedSprite2D.position = Vector2(-36,-38)
		...
		#Si se mueve vertical decidir que animacion usar
			...
			$AnimatedSprite2D.position = Vector2(32,-38)
			$AnimatedSprite2D.flip_h = false
			$StatusGood.position.x = -17
			$StatusGood.position.y = -36
			if walkingDown == true:
				$AnimatedSprite2D.position = Vector2(-8,-38)
